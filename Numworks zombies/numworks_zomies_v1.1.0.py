...
def collisions():
    ...
    for i in range (eQuantity):
        ...
        if pLeft <= powerUp[0] and pRight >= powerUp[0] and pTop <= powerUp[1] and pBottom >= powerUp [1]:
            powerUp = [-1000, -1000]
            announcer = True
            if powerUpType == 1:
                score += 10
...
sleep(5) #this is ONLY for debugging, as on pc the window closes with the program, so this is needed to see the end screen